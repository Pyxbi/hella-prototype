...
import { ArrowLeftIcon, ArrowRightIcon, ChatBubbleIcon, CheckIcon, GlobeIcon, MenuIcon, SearchIcon, SendIcon, SparkleIcon } from "../shared/icons";
...
function StoreIcon({ icon }: { icon: "web" | "shopee" | "tiktok" }) {
  if (icon === "web") return <GlobeIcon className="h-4 w-4" />;
  const src = icon === "shopee" ? `${IMG}/market_shopee.png` : `${IMG}/social_tiktok.png`;
  return <Image src={src} alt="" width={18} height={18} className="h-[18px] w-[18px] object-contain" />;
}
...
function History({ chats, activeId, onSelect, onNew }: {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-hella-cream px-5 py-7">
      ...
      <div className="mt-10 flex items-center gap-2 text-black"><SparkleIcon className="h-4 w-4 text-hella-green" /><span className="font-heading text-xl">Hella Sis</span></div>
      ...
      <nav aria-label="Lịch sử trò chuyện" className="min-h-0 flex-1 space-y-1 overflow-y-auto">
        {filtered.length ? filtered.map((chat) => (
          <button key={chat.id} onClick={() => onSelect(chat.id)} aria-current={chat.id === activeId ? "page" : undefined}
            className={`group flex w-full items-start gap-3 border-l-2 px-3 py-3 text-left transition-colors ${focus} ${chat.id === activeId ? "border-hella-green bg-white text-black" : "border-transparent text-black/75 hover:bg-white/60"}`}>
      ...
      <div className="mt-5 border-t border-black/10 pt-5">
        <p className="font-heading text-lg text-black">Một chút chăm sóc, mỗi ngày.</p>
  ...
  return (
    <article className="flex gap-3 sm:gap-5">
      ...
      <div className="min-w-0 flex-1">
        <p className="mb-3 font-heading text-lg text-black">Hella Sis</p>
        ...
        {!!message.answer.products?.length && <div className="mt-5 grid gap-3 sm:grid-cols-2">
          {message.answer.products.map((product) => <Link key={product.title} href={product.href} className={`group flex items-center gap-3 border border-black/10 bg-white p-3 transition hover:border-hella-green ${focus}`}>
            ...
            <div><p className="text-xs leading-5 text-black/80">{product.title}</p><span className="mt-2 inline-flex items-center gap-1 text-[11px] text-black/55 transition-colors group-hover:text-hella-green">Khám phá <ArrowRightIcon className="h-3 w-3" /></span></div>
          </Link>)}
        </div>}
        {!!message.answer.details?.length && <div className="mt-6 max-w-[680px] space-y-3 text-sm leading-7 text-black/70 sm:text-[15px]">
          {message.answer.details.map((detail) => <p key={detail}>{detail}</p>)}
        </div>}
        {!!message.answer.links?.length && <div className="mt-5 flex items-center gap-3">
          {message.answer.links.map((link) => <Link key={link.label} href={link.url} aria-label={link.label} title={link.label} className={`flex h-9 w-9 items-center justify-center rounded-full border border-black/10 bg-white text-black/55 transition hover:border-hella-green hover:bg-hella-cream hover:text-hella-green ${focus}`}><StoreIcon icon={link.icon} /></Link>)}
        </div>}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
        <div className="mx-auto flex min-h-full max-w-[840px] flex-col px-5 pb-4 pt-6 sm:px-10 sm:pt-6">
          {!chat ? <div className="flex flex-1 flex-col justify-center">
            <h1 className="font-heading max-w-[640px] text-3xl leading-[1.18] text-black sm:text-[40px]">Hôm nay, mình chăm sóc bạn thế nào?</h1>
            ...
            <p className="mt-5 text-xs text-black/50">Câu hỏi mẫu</p>
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              {suggestions.map((qa) => <button key={qa.id} onClick={() => send(qa.question)} className={`flex min-h-16 items-center justify-between gap-4 border border-black/10 p-3 text-left text-sm leading-6 transition hover:border-hella-green hover:bg-hella-cream ${focus}`}>{qa.question}<ArrowRightIcon className="h-4 w-4 shrink-0 text-black/45" /></button>)}
            </div>
            ...
            {chat.combo && <div className="border border-black/10 bg-hella-cream p-5 sm:ml-14">
              <p className="mb-3 text-xs text-black/55">Cùng tìm routine của bạn · {chat.combo.step + 1}/3</p>
        ...
        <div className="mx-auto max-w-[760px]">
          ...
          <form onSubmit={(e) => { e.preventDefault(); send(draft); }} className="border border-hella-green/35 bg-white p-4 shadow-[0_4px_24px_rgba(0,0,0,0.03)] focus-within:border-hella-green">
            ...
            <div className="mt-2 flex items-center justify-between">
              <span className="flex items-center gap-2 text-xs text-black/55"><SparkleIcon className="h-3.5 w-3.5 text-hella-green" /> Hella Sis</span>
      ...
      <section className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-[77px] shrink-0 items-center justify-between gap-3 border-b border-black/5 px-5 sm:px-10">
          <div className="flex min-w-0 items-center gap-3">
            <Button variant="ghost" size="icon" aria-label="Mở lịch sử trò chuyện" onClick={() => setMobileOpen(true)} className="rounded-none text-black hover:bg-hella-cream lg:hidden"><MenuIcon className="h-5 w-5" /></Button>
            <div className="min-w-0"><p className="font-heading text-2xl text-black">Hella Sis</p><p className="truncate text-[11px] text-black/50">Trợ lý làm đẹp dành riêng cho bạn</p></div>
          </div>
